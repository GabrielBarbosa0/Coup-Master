...
Correcao de ordem visual nos dois modos: o renderer guarda uma copia independente do ultimo estado exibido, impedindo que redesenhos de falas/idioma exponham mutacoes antecipadas. Revelacoes novas sao deduplicadas por sequencia e nunca puladas por timestamp vencido; cada carta recebe a duracao visual completa a partir da recepcao local. Atualizacoes recebidas durante o overlay ficam retidas ate terminar os eventos ainda nao vistos. A pausa transacional compartilhada permanece a mesma; clientes com atraso de rede podem concluir a apresentacao depois dela. Reentrada inicial nao reproduz historico. Teste: `ranked-reveal-order.test.js` cobre os dois renderers, atraso, referencias mutaveis, presenca e fila.
...
- `og:image` usa o icone quadrado em `assets/img/logo/favicon-coup-master.png`; os favicons das paginas usam a versao circular.
...
- `index.html`, `login.html`, `lobby.html`, `legal/privacy.html`, `legal/terms.html` e `landing.html` usam `og:image` com `assets/img/logo/favicon-coup-master.png`.
